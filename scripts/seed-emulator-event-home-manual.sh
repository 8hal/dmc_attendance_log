...
echo ""
echo "사전 닉(배번 자동 선택) — DevTools Console 후 새로고침:"
echo "  localStorage.setItem('dmc_attendance_v2_profile', JSON.stringify({nickname:'게살볶음밥',memberId:'seed_eh_member_gesal',team:'T1'}))"
echo "  → my-bib / event-home 에서「게살볶음밥」으로 바로 이어짐 (출석앱 프로필과 동일 키)"
