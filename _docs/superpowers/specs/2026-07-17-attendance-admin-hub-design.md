...
**기술 제약 (중요):**
- 동마클 카페는 **회원 전용**이라 비로그인 HTML fetch만으로는 본문이 안 나올 수 있음.
- 구현 후보 (justification 시 택1·승인):
  1. **본문 붙여넣기** — 카페에서 복사한 텍스트/HTML을 붙여 파싱 (쿠키 불필요, 1차 권장)
  2. **서버 fetch + 카페 세션/쿠키** (시크릿) — URL만으로 자동 (운영·보안 부담)
  3. **모바일 공유 텍스트** 수신
