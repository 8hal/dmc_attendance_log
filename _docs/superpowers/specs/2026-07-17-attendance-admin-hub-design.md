...
**보안 메모:** 회원 앱은 로그인 없이 localStorage 프로필이므로 self-delete는 **완전 방지 불가**. 완화: 당일만, rate limit, 서버에 memberId 우선 매칭, 감사 이벤트.
...
| 6 | 훈련 API | 1b 전 별도 승인 |
| 7 | 출석 삭제 | **개인 + 운영진 모두 필수.** 각각 신규 API + justification. Admin-1a는 조회만 가능 |
