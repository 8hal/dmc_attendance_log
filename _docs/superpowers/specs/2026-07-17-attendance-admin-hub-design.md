...
| 탭 | hash | 내용 | 출처 |
|----|------|------|------|
| **출석 관리** | `#attendance` | 날짜·정모 유형별 **당일 출석 현황** 조회 (인원·명단). 1차는 **읽기 중심** | 기존 `status` / `sessionCount` 재사용 |
...
| 권한 | verify-admin | 공개 |
| 보정(추가·삭제) | **Admin-1에서는 조회만** → Admin-2에서 검토 | 없음 |

### 4.2 Phase Admin-2 (이후)

| 탭/기능 | 용도 |
|---------|------|
| 출석 관리 — **수동 보정** | 누락 출석 추가·잘못 찍힌 행 삭제 (별도 승인·감사 로그 권장) |
...
**1차(Admin-1): 조회만.** 추가·삭제·수정은 Admin-2 + API 승인 후.  
데이터: 기존 `GET /attendance?action=status&date=` 및 `sessionCount` — **신규 API 없이** 시작 가능.
