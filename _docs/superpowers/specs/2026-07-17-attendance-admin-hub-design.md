...
**UI 스케치 (운영 입력) — 주간 일괄 (실무 반영 2026-07-17):**

운영진은 보통 **한 주(화·목·토)를 한 번에** 입력한다. 단일 슬롯만 두는 UI는 피로도가 높다.

| 패턴 | 동작 |
|------|------|
| **주간 보드** | 선택한 주의 화·목·토를 **3열(또는 세로 3카드)** 로 동시 편집 · 일괄 저장 |
| **시간·장소 기본값** | 요일별 기본: 화/목 `18:30`+장소, 토 `06:00`+장소 (`MEETING_INFO.md` / 마지막 저장값) |
| **전주 불러오기** | «지난주 내용 불러오기» → 같은 요일(화←화, 목←목, 토←토) 필드 복사. 시간·장소·전·본·후·서포터즈·메모 |
| **기본 = 전주** (권장) | 새 주 진입 시 해당 주 문서가 비어 있으면 **자동으로 전주 값을 폼에 프리필**(저장 전 편집 가능). 이미 저장된 주가 있으면 저장된 값 표시 |
| **요일만 복사** | (선택) «이 요일만 직전 동요일 불러오기» |

주 네비: `‹ 지난주 | 2026-07-14주 | 다음주 ›` + 일괄 «저장».

**회원 앱 소비:** 출석 셸 `#today`가 `(date, type)` 키로 조회 → 공지 레이아웃으로 표시.
...
**개인 삭제 UX (출석 셸):**  
- 오늘 탭: 이미 출석한 상태 → «출석 취소»  
- 내 출석: 해당 일 행 → 삭제(당일 제한 시 비활성 + 안내)
...
| 6 | 훈련 API | 1b 전 별도 승인 |
| 7 | 출석 삭제 | **개인 + 운영진 모두 필수.** 각각 신규 API + justification. 개인 삭제 = 당일·memberId 매칭 완화 (완전 보안 아님 — **합의됨**) |
