...
1. 문서에 `isRegularMember`가 없으면 → **`true` 설정** (`hidden` 여부와 무관)
2. 이미 `true`/`false`가 있으면 **유지** (`false`를 백필이 덮어쓰지 않음)
3. 보관(`hidden === true`) 문서도 missing → `true` (과거 정회원 이력). **백필이 `false`로 바꾸지 않음**
4. **「필드 없음 = 정회원」을 장기 기본으로 두지 않음.** 전이 기간 fallback은 허용 가능하나 목표는 **전원 명시 boolean**
5. **백필 전에** 비정회원으로 둘 사람(외부 추천 등)이 이미 `members`에 있으면 먼저 `false`로 두거나, 백필 대상에서 제외한다. 그렇지 않으면 규칙 1이 그 사람을 `true`로 만들 수 있음

### 구현 전 유의사항 (출석부)

필드를 넣고 `isRegularMember: false`인 문서를 `hidden: false`로 두면, **DMC 정모 출석부 API/UI를 아직 안 바꾸면 이름인 명부에 노출될 수 있다.**  
「필드 존재 = 정모에서 제외」가 **아님**. 노출 정책은 §6 후속.
