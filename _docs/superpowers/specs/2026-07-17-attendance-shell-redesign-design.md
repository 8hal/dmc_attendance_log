...
| 키오스크 접근 | 더보기에서 키오스크 모드 진입 (자봉·현장 태블릿) |
...
| **더보기** | `more` | 부가 진입·설정 | 프로필, 내 기록(my), 키오스크, 이용 안내 |
...
| **키오스크 모드** | `attendance-v2.html?mode=kiosk` 또는 in-app 전환 | 자봉·현장 태블릿. 탭 바 숨김 |
| 이용 안내 | 시트/모달 | 베타 안내, 정모 규칙 요약 |
| *(선택)* 동마클 홈 | `index.html` | Phase 1에서 생략 가능 |

**의도적으로 제외:** races(상단 전용), history(내 출석 탭이 대체), 레거시 전체 명단(팀 출석·키오스크가 대체).
...
1. 더보기에서 "키오스크 모드" 탭
2. 확인 다이얼로그 ("공용 기기에서 사용합니다. 개인 프로필이 숨겨집니다.")
3. `location`을 `?mode=kiosk`로 전환 (또는 hash 유지 + query 추가)
4. 키오스크 종료: 기존 blocker2 패턴의 "종료" → `mode` 제거 후 `#more` 복귀
...
| # | 항목 | 제안 기본값 |
|---|------|-------------|
| 1 | 레거시 `index.html` 컷오버 시점 | Shell-4 이후, 운영진 공지 후 |
| 2 | 팀 출석: 정모 외 모임 타입 | 1차 정모만, 필터는 2차 |
| 3 | `team-month-attendance` API | Shell-3 전 justification + 승인 |
| 4 | display 폰트 도입 | 1차 생략 |
| 5 | 더보기 «동마클 홈» 링크 | 1차 생략 |
...
- 수동: 오늘 체크인, 내 출석 월 이동, 팀 필터(내 팀/전체), 더보기→키오스크→종료, 상단→races
