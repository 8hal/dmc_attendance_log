...
2. **모듈은 독립, 네비는 하나.** 버스 탑승 · 배번 입력 · 결과 보드는 서로 전제 조건이 아니다. 버스가 꺼져 있어도 배번·결과 보드는 쓴다. 다만 **한 허브 페이지에서 서로 쉽게 이동**한다.
3. 버스만 대회별 on/off. 배번·결과 보드는 단체 대회면 기본 제공(기존 `my-bib`·취합 데이터에 의존).
4. 정회원 `participants`(배번·확정)와 버스 로스터(지인 포함)는 역할을 분리하고, `memberId`/닉네임으로 느슨히 연결한다.
5. **BE는 유지·확장, FE는 업데이트.** `group-detail` FE를 당일 UX 허브로 리팩터한다(전면 UI 정리 포함). 불필요한 BE 재작성은 하지 않는다.
6. 스펙은 전체 비전을 담고, **구현은 Phase로 나눈다**.
...
- 공개 배번 입력 (`my-bib`) — **버스 enable과 무관**
- 공개 결과 보드 (취합된 기록 읽기) — **버스 탑승 여부와 무관**
- **당일 UX 허브**: 버스·배번·결과 보드를 한 페이지에서 쉽게 이동
- **`group-detail.html` FE 리팩터** — 허브·정보 구조·모바일 운영 UX 정리 (BE는 기존 API 유지·버스 API만 추가)
...
- 버스 좌석 배정, 요금 결제
- `group-events` BE 전면 재작성 (버스 API 추가·필요 시 최소 수정만)
- 푸시 알림 / 카카오 알림톡
- 배번/결과 보드를 버스 로스터에 종속시키는 설계 (명시적 금지)
...
### 5.1 참가자 (공개) — 모듈 독립 + 상호 이동

| 화면 | URL (가칭) | Phase | 역할 |
|------|------------|-------|------|
| 당일 허브(참가자) | `group-day.html?eventId=` (가칭) 또는 공개용 허브 | 2 | 버스(활성 시) · 배번 · 결과 보드로 이동. **버스 off여도 배번·결과 진입 가능** |
| 탑승 체크 | `boarding.html?eventId=` | 1 | 닉네임 선택 → 구간별 탑승 완료. 비고 미표시. 하단/헤더에 배번·결과 링크 |
| 배번 입력 | `my-bib.html?eventId=` | 기존+2 | 버스와 무관. 헤더에 탑승(버스 on일 때)·결과 링크 |
| 결과 보드 | `group-results.html?eventId=` | 3 | 버스·배번 완료 여부와 **무관**. 확정/취합 기록. 헤더에 배번·(버스 on 시) 탑승 링크 |
...
상호 이동 규칙:
- 각 공개 화면은 동일 `eventId`로 다른 모듈 링크를 제공한다.
- 버스 모듈 링크는 `busBoarding.enabled === true` 일 때만 노출.
- 배번 링크는 단체 대회면 **항상** 노출 (Phase 1부터).
- 결과 보드 링크는 Phase 3 구현 후 노출. Phase 1–2 허브에는 결과 카드 자리만 두거나 ‘준비 중’으로 표시할 수 있으나, **버스/탑승으로 막지 않는다**.
...
| 당일 UX 허브 | `group-detail.html` | 1–2 (FE 리팩터) | 운영 허브: 기존 gap/배번/확정 + **버스·배번 공유·결과 보드** 카드. 버스는 항상 진입(미활성 시 활성화). 배번/결과 보드는 버스 상태와 무관하게 카드 노출 |
...
| Phase | 내용 | 성공 기준 |
|-------|------|-----------|
| **1** | `busBoarding` BE+공개/총무 탑승 화면, CSV·비고·QR, 서버 admin 검증. `group-detail`에 버스·배번·(결과 자리) **허브 카드 골격** | 철원형: CSV → 새벽 셀프체크 → 총무 미탑승·지인. 버스 off 대회에서도 배번 링크는 허브에 보임 |
| **2** | **`group-detail` FE 리팩터**(정보 구조·모바일·당일 허브 UX). 공개 화면 간 네비(탑승↔배번). 참가자 허브 정리 | 한 대회 페이지에서 모듈 이동이 분명. 버스 없이도 배번 사용 |
| **3** | `group-results.html` 공개 결과 보드 + 허브/공개 네비에 연결 (**버스·탑승 상태 비의존**) | 복귀 길·또는 버스 없는 대회에서도 동행 기록 확인 |
| **4** | 실시간 리스너, 중식 등 확장 (별도 승인) | — |

**구현 착수 순서:** Phase 1 계획 → 구현. Phase 2(FE 허브 리팩터)·3은 본 스펙을 유지한 채 별도 구현 계획.  
Phase 1에서도 `group-detail`에 **최소 허브 카드**는 넣어, 리팩터 전에 모듈이 고립되지 않게 한다.
...
목표: 단체 대회 **운영·당일 UX 허브**. BE 전면 재작성은 하지 않고, FE 정보 구조와 이동 경로를 업데이트한다.

### 11.1 해야 할 것

- 상단(또는 고정 영역)에 **모듈 카드/탭**: 참가자·gap·확정(기존) · **버스 탑승** · **배번 공유** · **결과 보드**.
- 버스: 총무는 항상 진입(미활성이면 활성화). `enabled`일 때 운영 중 뱃지·탑승 n/m.
- 배번·결과 보드: **`busBoarding`와 무관하게 항상 카드 노출**. 결과 보드 Phase 3 전이면 ‘준비 중’ 또는 숨김이 아니라, Phase 1–2에서는 배번만이라도 연결.
- 버스·탑승·결과 보드의 **본문 UI는 전용 HTML**로 두고, `group-detail`은 허브·요약·딥링크 역할 (파일 비대화 방지).
- 기존 gap/배번 편집/bulk-confirm 등 **동작 회귀 없이** 레이아웃·네비 중심으로 리팩터.
- 모바일(총무 새벽 사용) 우선: 큰 터치 타깃, 스크롤 한 번에 모듈 도달.

### 11.2 하지 않을 것

- `group-events` confirm/gap/participants API 재설계 (버그 수정 수준만).
- 버스 로스터를 `participants`에 합치기.
- 결과 보드·배번 API를 버스 enable에 묶기.
...
5. 참가자 공개 허브를 `group-day.html`로 둘지, `my-bib`/`boarding`/`group-results` 상호 링크만으로 충분할지 — Phase 2 계획 때 확정 (제품 원칙: 쉽게 이동).
...
- 버스가 없는 대회에서도 배번 입력·결과 보드를 쓸 수 있다.
- 한 허브(및 공개 화면 네비)에서 버스·배번·결과로 막힘 없이 이동한다.
- `group-detail` FE가 당일 운영 허브로 읽힌다 (BE는 안정 유지).
