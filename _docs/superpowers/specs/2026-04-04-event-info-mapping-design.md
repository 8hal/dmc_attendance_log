...
### 목표
1. **조기 대회 발견**: 기록 사이트에 등장하기 훨씬 이전에 대회 파악
   - 고러닝 등 정보 사이트는 대회 2~3개월 전부터 공지
   - 기록 사이트는 대회 당일~후에야 데이터 생성
   - → **사전 준비 활동 지원** (참가 독려, 카풀 조직, 응원 계획 등)
2. **대회 정보 소스 → race_events 연결** (고러닝 우선)
3. **확장 가능한 구조** (미래에 네이버스포츠 등 추가 시)
4. **여러 소스로 교차 검증** (robust한 진실 발견)
...
```
T-60일: 고러닝에 대회 공지 등장
        ↓ event_info_mappings 생성
        ↓ race_events 스텁 생성 (sourceMappings 비어있음)
        ↓
        ✅ 회원들에게 대회 안내 가능
        ✅ 참가 독려, 카풀 조직, 응원 계획
        ✅ 훈련 일정 조정
