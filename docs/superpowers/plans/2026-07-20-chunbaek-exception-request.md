...
- 회원이 기간 예외를 **요청·조회·취소·조기해제**할 API 없음 확인
...
| cancel-exception-request | pending 취소 |
...
**`handleCancelExceptionRequest`** (POST): `requestId` body, 본인·`status===pending`만 → `cancelled` + `updatedAt`
...
if (action === "cancel-exception-request") return handleCancelExceptionRequest(req, res, db);
...
async function cancelExceptionRequest(requestId) {
  return apiPost("cancel-exception-request", { requestId });
}
...
- pending 행에 취소 버튼 → `cancelExceptionRequest`
