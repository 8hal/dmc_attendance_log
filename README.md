...
### GET /attendance?action=nicknames&limit=500

닉네임 목록 조회 (자동완성용)

- limit: 최대 개수 (기본: 500, 최대: 1000)
- TEST로 시작하는 닉네임 자동 제외
- 가나다순 정렬
...
### 2026. 1. 22. - 닉네임 자동완성 기능 ✨

- **닉네임 자동완성 UI 추가**
  - 입력 시 기존 닉네임 목록에서 매칭되는 닉네임 제안
  - 커스텀 드롭다운 UI (모바일/데스크톱 모두 지원)
  - 매칭 텍스트 하이라이트
  - 키보드 네비게이션 지원 (↑↓, Enter, Esc)
- **새 API 추가**: `GET /attendance?action=nicknames`
  - 전체 닉네임 목록 조회 (자동완성용)
  - TEST 닉네임 자동 제외, 가나다순 정렬
- **성능 최적화**: 페이지 로드 시 병렬 API 호출 (status + nicknames)
